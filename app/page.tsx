...
import { Trophy, Globe, HelpCircle, Pickaxe, Activity } from 'lucide-react';
...
export default function SonicMinerPage() {
  ...
  return (
    <main className="min-h-screen bg-neutral-950 text-white flex flex-col items-center p-4 font-sans overflow-hidden relative selection:bg-purple-500/30">
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8 z-10 flex-1">
        ...
        <div className="flex flex-col items-center justify-center relative min-h-[500px]">
            ...
            <div className="absolute top-24 flex flex-col items-center text-center">
                 <div className="text-xs text-gray-500 font-bold uppercase mb-1">Your Session Score</div>
                 <div className="text-5xl font-black font-mono text-white tabular-nums tracking-tight">{gameState.myScore.toLocaleString()}</div>
            </div>

            <GameControls onDig={handleDig} disabled={false} />
        </div>

        {/* RIGHT: LIVE FEED */}
        <div className="flex flex-col h-[600px] bg-neutral-900/30 rounded-3xl border border-white/5 overflow-hidden backdrop-blur-sm relative">
            <div className="p-4 border-b border-white/5 flex justify-between items-center bg-neutral-900/50">
                <span className="text-xs font-bold uppercase tracking-wider text-gray-400 flex items-center gap-2">
                    <Activity className="w-3 h-3 text-blue-400" /> Sonic Network Live
                </span>
                <div className="flex items-center gap-1.5 text-[10px] font-mono text-green-400 bg-green-500/10 px-2 py-0.5 rounded-full border border-green-500/20">
